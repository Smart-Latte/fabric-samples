...
func Buy(contract *client.Contract) {
	...
	for _, energy := range energies {
		...
		if distance <= searchRange && auctionStartTimeCompare.After(energy.AuctionStartTime) == false {
			...
			fmt.Printf("id:%s, latitude: %g, longitude:%g, unitPrice:%g, distance:%g, auctionStartTime:%v it's invalid.\n",
	...
	for i := 0; i < tokenNum ; i++ {
		fmt.Println(validEnergies[i])
		massage := bidOnToken(contract, validEnergies[i].ID, validEnergies[i].BidPrice)
		If (massage == "your bid was successful") {
			successBid = append(successBid, validEnergies[i])
			// auctionstart + 5min 経ったら見に行く
		}
	}
	// 失敗した数だけ再チャレンジ
	// 成功したものは、auctionstart + 5min 経ったら見に行く
*/
...
func bid(contract *client.Contract, energies []Energy, bidNum int) []Energy {
	...
	// 失敗した数だけ再チャレンジ
	// 成功したものは、auctionstart + 5min 経ったら見に行く。まとめてでもいいかも
}
