...
func Create(contract *client.Contract) {
	...
	id := timestamp.Format(layout) + username + "-" + rand.Intn(10000)

	var energy Energy

	//for i := 0; i < createTokenNum; i++ {
	energy = createToken(contract, id, timestamp, largeCategory, smallCategory)
...
loop:
	for {
		...
		case <-ticker.C:
			count++
			fmt.Printf("count:%d\n", count)
			auctionenTimestamp := timestamp.Add(time.Minute * time.Duration(count*auctionEndInterval))
			massage := auctionEnd(contract, energy.ID, auctionenTimestamp)
			fmt.Printf("timestamp:%v, id:%s\n", auctionenTtimestamp, energy.ID)
			...
			if massage == stopmassage1 || massage == stopmassage2 || massage == stopmassage3 {
				ticker.Stop()
				break loop
			} else if massage == "Why did you call this function?" && count == 4 {
				...
				break loop
			} else if massage == "Why did you call this function?" && count == 5 {
				ticker.Stop()
				fmt.Println("final")
				break loop
...
func discountUnitPrice(contract *client.Contract, enegyId string) error {
	return contract.SubmitTransaction("DiscountUnitPrice", energyId)
}
